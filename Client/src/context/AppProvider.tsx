...
import {PurchaseProductAddDTO} from "../interfaces/purchaseAddDTO.ts";
...
export const AppContextProvider: React.FC<AppContextProviderProps> = ({
  ...
  const fetchCategories = async () => {
    try {
      ...
      setActualCategory(response.data.body.categories[0]);
  ...
  const payMercadoPago = async (idPurchase: number, purchaseProducts: PurchaseProductAddDTO[]) => {
    const response = await createOrderMp(idPurchase, purchaseProducts);
  ...
  const goToWpp = (nroPurchase:number) => {
    ...
    const numeroWhatsApp = import.meta.env.VITE_WPP ?? window._env_.WPP;
  ...
  const pay = async (payment: string, customerId: number) => {
    ...
    const purchaseProducts:PurchaseProductAddDTO[] = carrito.map(p=>{
      return{
        productId: p.id,
        quantity: p.quantity
      }
    ...
    try {
      ...
      if (payment == "MERCADO_PAGO") payMercadoPago(response.data.body.id, purchaseProducts);
