...
export default function PayButton() {
  ...
  const [payment, setPayment] = useState<"MERCADO_PAGO" | "CASH">("MERCADO_PAGO");
  ...
  const onSubmitWithoutAccount = async () => {
    ...
    console.log(formData)
      try {
        ...
        const { data } = response;
        if (data.ok == true) {
          localStorage.setItem("token", data.body.jwtToken);
              ...
              <FormControl mt={4}>
                <FormControl mb={6}>
                  ...
                  <Flex gap={2} alignItems={"center"}>
                    <Button
                      type="button"
                      variant={payment === "MERCADO_PAGO" ? "solid" : "outline"}
                      colorScheme={payment === "MERCADO_PAGO" ? "blue" : "gray"}
                      onClick={() => setPayment("MERCADO_PAGO")}
                    >
                      <Img
                        rounded={"xl"}
                        w={45}
